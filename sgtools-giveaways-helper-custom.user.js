...
(function () {
  ...
  var USER_NAME = $(".nav__avatar-outer-wrap").attr("href").replace("/user/", "");
  ...
  if (current_path) {
	...
	} else if (current_path.length !== 0 && current_path[0] === 'group') {
	  addUserQuicksearch(current_path);
	}
  }
  ...
  function addUserQuicksearch(current_path) {
	// for example: /group/ruzuL/vikinggiveaways/users/search?q=barokai
	var href = "/" + current_path[0] + "/" + current_path[1] + "/" + current_path[2] + "/users/search?q=" + USER_NAME.toLowerCase();
	// short but ugly
	//$("ul.sidebar__navigation").first().append('<li class="sidebar__navigation__item"><a href="#" class="sidebar__navigation__item__link"><i class="fa fa-caret-right"></i><div class="sidebar__navigation__item__name">Users</div><div class="sidebar__navigation__item__underline"></div></a></li>');

	// better readable
	$("ul.sidebar__navigation").first().append(
	  $('<li>').attr('class', 'sidebar__navigation__item').append(
		$('<a>').attr({
		  href: href,
		  class: 'sidebar__navigation__item__link'
		}).append(
		  $('<i>').attr('class', 'fa fa-search'),
		  $('<div>').attr('class', 'sidebar__navigation__item__name').append("Quicksearch for " + USER_NAME),
		  $('<div>').attr('class', 'sidebar__navigation__item__underline'))));

	// append user info to group header
	var userSearchUrl = "https://www.steamgifts.com" + href;
	$.get(userSearchUrl, function (content) {
	  var table = $("div.table", content);
	  // light gray background for table row
	  $('.table__row-inner-wrap', table).css('background-color', '#E8EAEF');
	  $("div.featured__summary").append(table);
	});
  }
  ...
  function giveawayNew() {
	let getGroupButtons = function () {
	  ...
	  buttons += '&nbsp;<button type="button" class="btn btn-sm btn-warning" id="btnMulti"><i class="fa fa-user-times"></i>&nbsp; Create multi</button>';
	  return buttons;
	};
	...
	let applySettings = function (group) {
	  ...
	  $(".form__group.form__group--steam").hide();
	  $(".form__group.form__group--steam.is-selected").show();
	  // focus input to enter game name
	  $(".js__autocomplete-name").focus();
	};

	let applyMulti = function () {
	  // set to 4 days
	  SGTOOLS_TIME = SGTOOLS_TIME * 2;
	  applyDates();
	  applyRegionRestrictions();
	  var description = "# Multi Group GA time!\n##For following groups (+whitelist):\n";

	  var multiGroup = [GROUPS.gergive, GROUPS["AUT-SG"], GROUPS.boxofkittens];
	  $.each(multiGroup, function (key, g) {
		applyPrivate(g);
		applyLevel(g);
		applyGroup(g);
		description += "\n* " + g.name;
	  ...
	  // add whitelist
	  if ($('div.form__group.form__group--whitelist.is-selected').length === 0) {
		$("div.form__group.form__group--whitelist").click();
	  }
	  // hides not used checkboxes
	  $(".form__checkbox.is-disabled").hide();
